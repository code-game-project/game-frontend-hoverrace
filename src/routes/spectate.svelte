<script lang="ts">
	...
	const resize = () => {
		...
		height = isFullscreen
			...
			: canvasContainerHeight - controlsHeight;
	...
	const initStatus = () => {
		status = new Text(WAITING_MSG, {
			fill: '#00ff99',
			fontSize: 0.06 * pixiApp.screen.height,
			align: 'center',
		});
		status.roundPixels = true;
		status.anchor.set(0.5);
		status.y = -400;
		status.x = pixiApp.screen.width / 2;
		map.addChild(status);
	};
